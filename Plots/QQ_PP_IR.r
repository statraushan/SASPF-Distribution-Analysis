...
############################
data <- DataSetsUni::data_increaserate
eps <- 1e-6
...
pdf_list <- list(
  Custom = list(pdf = pdf_custom, theta = 0.437850381),
  TL     = list(pdf = pdf_TL, theta = 1.018055328),
  UNXLD  = list(pdf = pdf_UNXLD, theta = 1.0348277),
  PD     = list(pdf = pdf_PD, theta = 0.707569272),
  Kw     = list(pdf = pdf_Kw, theta = 0.574, beta = 0.737),
  BD     = list(pdf = pdf_Beta, theta = 0.599, beta = 0.736)
)

cdf_list <- list(
  Custom = function(x) cdf_custom(x, 0.437850381),
  TL     = function(x) cdf_TL(x, 1.018055328),
  UNXLD  = function(x) cdf_UNXLD(x, 1.0348277),
  PD     = function(x) cdf_PD(x, 0.707569272),
  Kw     = function(x) cdf_Kw(x, 0.574, 0.737),
  BD     = function(x) cdf_Beta(x, 0.599, 0.736)
)
...
pdf("QQ_plot-IR.pdf", width = 6, height = 5)
grid.arrange(grobs = qq_plots, ncol = 2)
dev.off()

pdf("PP_plot-IR.pdf", width = 6, height = 5)
